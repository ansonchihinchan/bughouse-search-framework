...
TEST_CASE("PieceSquareEvaluator mirrors PAWN_PST vertically for Black",
  ...
  int white_score =
      eval.evaluate(make_classical_context(white_advanced)).mid_game();
  int black_score =
      eval.evaluate(make_classical_context(black_advanced)).mid_game();
...
TEST_CASE("PieceSquareEvaluator uses phase-dependent king tables (mid vs "
  ...
  EvalScore score = eval.evaluate(make_classical_context(centred_king));
