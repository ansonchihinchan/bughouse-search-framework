...
TEST_CASE(
    "make_classical_context computes maximum phase for the standard start "
    "position",
    "[eval][types][phase]") {
  Board board;
...
TEST_CASE("make_classical_context computes zero phase for bare kings",
          "[eval][types][phase]") {
  Board board;
...
TEST_CASE(
    "make_classical_context flags an unopposed pawn as passed and isolated",
...
TEST_CASE(
    "make_classical_context does not flag a pawn as passed when blocked by "
    "an enemy "
...
TEST_CASE(
    "make_classical_context flags pawns doubled on the same file for both",
    "[eval][types][pawn_info]") {
  Board board;
...
TEST_CASE(
    "make_classical_context computes attack_info.kingZone as king_attacks "
    "plus the "
    "king's own square",
...
TEST_CASE("make_classical_context computes non-zero attacks for the side with "
          "pieces and "
