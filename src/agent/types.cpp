...
protected:
  ...
  std::optional<Message> make_outgoing_message(
      const SearchResult &result, const BughousePosition &resulting_position,
      PlayerId player,
      const std::array<int64_t, PLAYER_NO> &remaining) override {
    if (result.best_move.is_none())
      return std::nullopt;
    Message message = generator_.generate_message(result, resulting_position,
                                                  player, remaining);
...
class SacrificeAgent : public Agent {
public:
  explicit SacrificeAgent(AgentConfig config)
      : Agent(config, std::make_unique<BughouseEvaluator>(
                          BughouseEvaluationConfig::independent())),
        coordinator_(config.temporal_config) {}

  AgentType type() const override { return AgentType::Sacrifice; }
  std::string_view name() const override { return "sacrifice"; }

protected:
  SearchResult select_move(const BughouseState &game,
    ...
    SearchResult result =
        coordinator_.select_move(game, context.root_player, baseline, limits,
                                 stop_token, decision_start);
    const TemporalDecision &decision = coordinator_.last_decision();
    last_metrics_.sacrifice_attempts =
        decision.stats.sacrifice_candidates_considered;
    last_metrics_.sacrifices_accepted = decision.selected_sacrifice ? 1 : 0;
    last_metrics_.temporal_transfers_observed =
        decision.stats.transfers_observed;
    last_metrics_.temporal_partner_uses =
        decision.stats.transferred_pieces_used;
    return result;
  }

private:
  TemporalCoordinator coordinator_;
};
} // namespace
...
std::optional<Message> Agent::make_outgoing_message(
    const SearchResult &result, const BughousePosition &resulting_position,
...
SearchResult Agent::select_move(const BughouseState &game,
                                const SearchContext &context,
                                const SearchLimits &limits,
                                std::stop_token stop_token) {
  return search_->search(game.position, context, limits, stop_token);
}
...
AgentOutput Agent::choose_move(const BughouseState &game, PlayerId player,
  ...
  SearchResult result = select_move(game, context, limits, stop_token);
  ...
  std::optional<Message> outgoing =
      make_outgoing_message(result, resulting_position, player, remaining);
  return AgentOutput{std::move(result), std::move(outgoing), last_metrics_};
}
...
std::unique_ptr<Agent> make_agent(const AgentConfig &config, Channel *channel) {
  ...
  case AgentType::Sacrifice:
    return std::make_unique<SacrificeAgent>(config);
  }
