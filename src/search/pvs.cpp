...
int PVS::search_tail_move(BughousePosition &position, const SearchContext &next,
                          ...
                          int reduction, std::stop_token stop_token) {
  int score = -alpha_beta(position, next, depth - 1 - reduction, -alpha - 1,
                          -alpha, ply + 1, stop_token);

  if (reduction > 0 && score > alpha)
    score = -alpha_beta(position, next, depth - 1, -alpha - 1, -alpha, ply + 1,
                        stop_token);
